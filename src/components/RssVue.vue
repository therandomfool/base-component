...
import Item from "./child/ItemPanel.vue";
import DivisionSelect from "./child/DivisionSelect.vue";
import Loading from "./child/LoadingPage.vue";

const selected = ref("nfcwest");
...
async function getData() {
  ...
  if (rss) {
    ...
    if (!data[0].url) {
      data = data.map((story: { enclosure: { [x: string]: { url: any } } }) => {
        const url = story.enclosure?.["$"]?.url;
        return { ...story, url };
      });
      localStorage.setItem(
        `espn-nfl-rss-${selected.value}`,
        JSON.stringify(data)
      );
    }
    items.value = data;
  ...
  try {
    ...
    items.value = rss.map(
      (story: { enclosure: { [x: string]: { url: any } } }) => {
        const url = story.enclosure?.["$"]?.url;
        return { ...story, url };
      }
    );
